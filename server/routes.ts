...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/grid-search", async (req: Request, res: Response) => {
    try {
      ...
      // Persist report summary if projectId is provided
      const { projectId } = req.body;
      if (projectId && user.userId) {
        try {
          updateProject(user.userId, projectId, {
            lastRunAt: Date.now(),
            lastSummary: {
              avgRank: avgRank ? Math.round(avgRank * 10) / 10 : null,
              found: rankedResults.length,
              totalPoints: gridPoints.length,
              top3: top3Count,
              pos4_10: top10Count - top3Count,
              pos11_20: top20Count - top10Count,
              notFound: gridPoints.length - rankedResults.length
            }
          });
          console.log(`Updated project ${projectId} with latest report summary`);
        } catch (persistError) {
          console.error("Failed to persist project summary:", persistError);
        }
      }
  ...
  app.post("/api/geo-grid/projects/sync", async (req, res) => {
    try {
      const user = (req as any).session.user || { userId: 'dev-user' };
      const filePath = path.join(process.cwd(), "data", "geo-grid-projects.json");
      ...
      const projects = data[user.userId] || [];
  ...
  app.get("/api/geo-grid/projects", async (req, res) => {
    try {
      const user = (req as any).session.user || { userId: 'dev-user' };
      const filePath = path.join(process.cwd(), "data", "geo-grid-projects.json");
      ...
      console.error("Error fetching geo-grid projects:", error);
  ...
  app.get("/api/geo-grid/projects/:id", async (req, res) => {
    try {
      ...
      console.error("Error fetching geo-grid project:", error);
      return res.status(500).json({ error: "Failed to fetch project" });
    }
  });

  app.delete("/api/geo-grid/projects/:id", async (req, res) => {
    try {
      const user = (req as any).session.user || { userId: 'dev-user' };
      const { id } = req.params;

      const success = deleteProject(user.userId, id);
      ...
      console.error("Error deleting geo-grid project:", error);
