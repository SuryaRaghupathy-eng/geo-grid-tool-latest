...
export default function Step5Review() {
  ...
  const handleCreateCampaign = async () => {
    ...
    const campaignData = {
      ...
      name: `${searchKeyword} - ${selectedLocation?.address?.split(',')[0] || 'Report'}`,
      keyword: searchKeyword,
      location: selectedLocation?.address,
      website: businessWebsite,
      grid: {
        size: `${gridConfig?.gridSize}x${gridConfig?.gridSize}`,
        spacing: `${gridConfig?.spacing}${gridConfig?.distanceUnit === "miles" ? "mi" : "m"}`
      },
      selectedLocation,
      gridConfig,
    ...
    // Store for both dashboard and report page
    sessionStorage.setItem("campaignData", JSON.stringify(campaignData));
    sessionStorage.setItem("currentProjectId", campaignData.id);
    
    // Crucial: report-page uses "reportData" specifically
    const reportData = {
      keyword: searchKeyword,
    ...
    try {
      ...
      if (!response.ok) {
        console.error("Failed to sync project:", response.status);
      }
    } catch (error) {
