...
export default function ReportPage() {
  ...
  const calculateGridPointsFromProject = (project: any): GridPoint[] => {
    ...
    const half = Math.floor(project.gridConfig.gridSize / 2);
    const spacingInMeters = project.gridConfig.distanceUnit === "miles" ? project.gridConfig.spacing * 1609.34 : project.gridConfig.spacing;
    const latOffset = spacingInMeters / 111320;
    const lngOffset = spacingInMeters / (111320 * Math.cos(project.selectedLocation.lat * Math.PI / 180));
    
    for (let row = -half; row <= half; row++) {
      for (let col = -half; col <= half; col++) {
        const lat = project.selectedLocation.lat + (row * latOffset);
        const lng = project.selectedLocation.lng + (col * lngOffset);
        ...
        points.push({
          ...
          row,
          col,
          isSelected: project.selectedPointIds?.includes(`${row}_${col}`) ?? true,
          isCenter,
        });
      }
    }
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {reportData.keyword || sessionStorage.getItem("searchKeyword") || "No keyword"} - Generated on {new Date(reportData.createdAt).toLocaleString()}
                </p>
              </div>
            </div>
